...
void add_line_restart(MENU_ITEM *menu_item)
{
    if (eq(get_newmitem(), ADD_OBJECTS_UNDO))
    {
        ...
        if (get_line_connected() == CONNECTED)
        {
            ...
            if ((object->visibility == GINVISIBLE) && (previous_end_pt != (Gpoint *) NULL))
            {
...
void add_curve_restart(MENU_ITEM *menu_item)
{
    ...
    if (eq(get_newmitem(), ADD_OBJECTS_UNDO))
    {
        ...
        if (get_line_connected() == CONNECTED)
        {
            ...
            if ((object->visibility == GINVISIBLE) && (prev_control[0].x != -1.0))
            {
                control[0] = past_control[3];
                control[1].x = control[0].x + (control[0].x - past_control[2].x);
                control[1].y = control[0].y + (control[0].y - past_control[2].y);
            ...
            else /* last object just un-erased */
            {
                ...
                control[1].x = control[0].x + (control[0].x - prev_control[2].x);
                control[1].y = control[0].y + (control[0].y - prev_control[2].y);
                prompt(38);
            }
        }
        else /* line_connected_state == NOT_CONNECTED */
...
void add_text_exec(Gpoint pt)
{
    ...
    if ((ch == END_OF_STR) && (head_char == (CHAR_OB *) NULL))
        prompt(12);
...
void switch_line_connected_state(POPUP *popup, Gpoint pt)
{
    ...
    {
    case CONNECTED:
        if ((pt_needed == FIRST_PT) && (object != (OBJECT *) NULL))
        {
            if (object->visibility == GVISIBLE)
