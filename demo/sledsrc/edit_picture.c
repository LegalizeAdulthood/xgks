...
void edit_picture_undo(MENU_ITEM *menu_item)
{
    ...
    if ((new_list != (COMB_OB_PTR *) NULL) || (undo_list != (COMB_OB_PTR *) NULL))
    {
        hilite(menu_item->key);
        ...
        if ((new_list != (COMB_OB_PTR *) NULL) || ((undo_list != (COMB_OB_PTR *) NULL) && (get_update_mode() != QUICK_UPDATE)))
        {
            redraw_objects();
        }
        else if ((undo_list != (COMB_OB_PTR *) NULL) && (get_update_mode() == QUICK_UPDATE))
