...
void adjust_tuner_clr(void)
{
    ...
    hsv_clr.hue = 360 * (hue_arrow.knob_pos - hue_arrow.mini) / (hue_arrow.maxi - hue_arrow.mini);
    hsv_clr.val = (val_arrow.knob_pos - val_arrow.mini) / (val_arrow.maxi - val_arrow.mini);
    hsv_clr.sat = (sat_arrow.knob_pos - sat_arrow.mini) / (sat_arrow.maxi - sat_arrow.mini);
    hsv_to_rgb(&hsv_clr, &rgb_clr);
    set_color(tuner_clr, &rgb_clr);
} /* adjust_tuner_clr */
...
int arrow_picked(ARROW arrow, Gpoint pt)
{
    ...
    {
        if ((arrow.mini <= pt.y) && (arrow.maxi >= pt.y) && (abs(arrow.con - pt.x)
                                                                <= (ARROW_LEN * 2.0)))
        {
            return (TRUE);
        }
    ...
    {
        if ((arrow.mini <= pt.x) && (arrow.maxi >= pt.x) && (abs(arrow.con - pt.y)
                                                                <= (ARROW_LEN * 2.0)))
        {
            return (TRUE);
        }
