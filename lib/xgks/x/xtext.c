...
static void xComputeText(WS_STATE_PTR ws, TEXT_ST *tx)
{
    ...
    stroke_trans[0] = (sndc_wt.x) / (FontTable[wsr_font].f_font->fnominalx);
    stroke_trans[3] = (sndc_wt.y) / (FontTable[wsr_font].f_font->fnominalx);
    stroke_trans[1] = (sndc_ht.x) / (FontTable[wsr_font].f_font->fnominaly);
    stroke_trans[4] = (sndc_ht.y) / (FontTable[wsr_font].f_font->fnominaly);
...
Gint xXgksText(WS_STATE_PTR ws, TEXT_ST *tx)
{
    ...
    case GTP_DOWN:
        ...
        case GTV_CAP:
            ...
            break;
        case GTV_HALF:
            ytrans = fsize / 2.0 - xFyMax(&(cfont->fchars[cfont->fcharoffset[*(tx->string)]]));
            break;
        case GTV_BASE:
            ytrans = fsize - xFyMax(&(cfont->fchars[cfont->fcharoffset[*(tx->string)]]));
            break;
        case GTV_BOTTOM:
    ...
    for (text_ptr = tx->string; *text_ptr; text_ptr++)
    {
        ...
        case GTP_RIGHT:
            ...
            break;
        case GTP_LEFT:
            xtrans = -xFxMax(&(cfont->fchars[cfont->fcharoffset[*(text_ptr + 1)]]))
                - cfont->fnominalx * wsr_spacing;
            ytrans = 0;
            break;
        case GTP_UP:
            ytrans = xFyMax(&(cfont->fchars[cfont->fcharoffset[*text_ptr]])) - xFyMin(&(cfont->fchars[cfont->fcharoffset[*(text_ptr + 1)]]))
                + cfont->fnominaly * wsr_spacing;
            xtrans = 0;
            break;
        case GTP_DOWN:
            ytrans = -xFyMax(&(cfont->fchars[cfont->fcharoffset[*(text_ptr + 1)]])) + xFyMin(&(cfont->fchars[cfont->fcharoffset[*(text_ptr)]]))
                - cfont->fnominaly * wsr_spacing;
            xtrans = 0;
            break;
        }
...
static void xPlotStrokeChar(struct vcharst *cvp)
{
    ...
    { /* get the next stroke */
        ...
        case 'm':
            ...
            tmp.x = cvp->vc_x;
            tmp.y = cvp->vc_y;
            pt->x = (int) (stroke_trans[0] * tmp.x + stroke_trans[1] * tmp.y + stroke_trans[2] + 0.5);
...
#ifdef TDEBUG
static int xDumpMatrix(s, m) char *s;
Gfloat m[];
{
    int i;
