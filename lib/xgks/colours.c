...
Gcobundl *rep;
{
    ...
    GKSERROR(((rep->red < 0.0 || rep->red > 1.0) || (rep->green < 0.0 || rep->green > 1.0) || (rep->blue < 0.0 || rep->blue > 1.0)), 96, errgsetcolourrep);
...
Gint ginqcolourrep(Gint ws_id, Gint idx, Gqtype type, Gcobundl *rep)
{
    ...
    /* idx has been set....                               */
    if (type == GSET && ws->set_colour_rep != NULL && ws->set_colour_rep[idx].red >= 0.0)
    { /* give him SET... */
...
Gint ginqpredcolourrep(Gchar *ws_type, Gint index, Gcobundl *rep)
{
    ...
    {
        if (xgks_state.openedws[i].ws_id == INVALID || xgks_state.openedws[i].ws->ewstype != X_WIN)
            continue;
