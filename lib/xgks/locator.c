...
#define TRANSFORM xgks_state.ntrans_list
    ...
    GKSERROR((init->position.x < TRANSFORM[init->transform].ntrans.w.xmin || init->position.x > TRANSFORM[init->transform].ntrans.w.xmax || init->position.y < TRANSFORM[init->transform].ntrans.w.ymin || init->position.y > TRANSFORM[init->transform].ntrans.w.ymax),
        152, errginitloc);
...
#define LNBUNDLE ws->lnbundl_table[record->pet4.ln.line]
        gcvalues.function = GXinvert;
        gcvalues.foreground = (record->pet4.ln.colour == GBUNDLED) ? LNBUNDLE.colour : record->pet4.ln.bundl.colour;
        if (!WS_AVAIL_COLOUR(ws, (int) gcvalues.foreground))
            gcvalues.foreground = ws->wsfg;
        gcvalues.background = ws->wsbg;
        gcvalues.line_width = (int) ((record->pet4.ln.width == GBUNDLED) ? LNBUNDLE.width : record->pet4.ln.bundl.width);
        gcvalues.line_style = ((record->pet4.ln.type == GBUNDLED) ? LNBUNDLE.type : record->pet4.ln.bundl.type);
        ...
        gcvalues.line_style = (gcvalues.line_style == GLN_SOLID) ? LineSolid : LineOnOffDash;
#ifdef LOCDEBUG
...
#define LNBUNDLE ws->lnbundl_table[record->pet5.attr.ln.line]
            gcvalues.function = GXinvert;
            gcvalues.foreground = (record->pet5.attr.ln.colour == GBUNDLED) ? LNBUNDLE.colour : record->pet5.attr.ln.bundl.colour;
            if (!WS_AVAIL_COLOUR(ws, (int) gcvalues.foreground))
                gcvalues.foreground = ws->wsfg;
            gcvalues.background = ws->wsbg;
            gcvalues.line_width = (int) ((record->pet5.attr.ln.width == GBUNDLED) ? LNBUNDLE.width : record->pet5.attr.ln.bundl.width);
            gcvalues.line_style = ((record->pet5.attr.ln.type == GBUNDLED) ? LNBUNDLE.type : record->pet5.attr.ln.bundl.type);
            didx = gcvalues.line_style;
            if (didx < 0)
                didx += 3;
            else if (didx > 0)
        ...
        else if (record->pet5.pfcf == GPF_FILLAREA)
        {
            if (record->pet5.acf == GSPECIFIED)
            {
                ...
                GKSERROR(!WS_FILL_TYPE(record->pet5.attr.fl.bundl.inter,
                             record->pet5.attr.fl.bundl.style),
                    85, errginitloc);
...
#define FLBUNDLE ws->flbundl_table[record->pet5.attr.fl.fill]
            gcvalues.function = GXinvert;
            gcvalues.foreground = (record->pet5.attr.fl.colour == GBUNDLED) ? FLBUNDLE.colour : record->pet5.attr.fl.bundl.colour;
            if (!WS_AVAIL_COLOUR(ws, (int) gcvalues.foreground))
                gcvalues.foreground = ws->wsfg;
            gcvalues.background = ws->wsbg;
            gcvalues.line_width = 0;
