...
} PLINE_ST;                     /* Output Primitive structure for polyline */
...
} PMARK_ST;                     /* Output Primitive structure for polymarker */

typedef struct {
        Gchar   *string;        /* The string to be output */
} MESG_ST;                      /* Structure for implementation dependent message */

typedef struct {
        Gpoint  *location;      /* Starting location of text */
        Gchar   *string;        /* The string to be output */
        Gpoint  up_vec, base_vec; /* Orientation and magnituce in NDC space */
        Gtxattr txattr;         /* Text attributes */
        CHATTR  chattr;
} TEXT_ST;                      /* Output Primitive structure for text */
...
} FILL_AREA_ST;                 /* Output primitive structure for fill area */

typedef struct {
        Gpoint ll, lr,
               ur, ul;          /* the NDC values of the cell rectangle */
        Gipoint dim;            /* colour index array dimensions */
        Gint rowsize;           /* colour index array row length */
        Gint *colour;           /* colour index array */
} CELL_ARRAY_ST;                /* Outptu promitive structure for cell array */
...
} GDP_ST;                       /* Output primitive structure for generous purpose primitive */

typedef struct entry {
        PID     pid;            /* Identify which type of output primitive is active */
        Gint    seg_cnt;        /* Segment counter to non_segment primitives */
        Gint    pickid;         /* Identify which of the primitives in the segment is picked */
        struct entry *next;     /* Pointer to next output primitive structure */
        union {
                PLINE_ST        pline;
                PMARK_ST        pmark;
                TEXT_ST         text;
